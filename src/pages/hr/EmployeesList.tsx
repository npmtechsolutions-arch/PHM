...
export default function EmployeesList() {
    ...
    useEffect(() => {
        fetchEmployees();
    }, [deptFilter, currentPage, pageSize, search, statusFilter, activeEntity]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const payload: any = {
                ...
                shop_id: activeEntity?.type === 'shop' ? activeEntity.id : (formData.shop_id || undefined),
                warehouse_id: activeEntity?.type === 'warehouse' ? activeEntity.id : undefined,
    ...
    return (
        <UniversalListPage loading={mastersLoading}>
            <UniversalListPage.Header
                title={
                    activeEntity 
                        ? `Employees - ${activeEntity.type === 'shop' ? 'Pharmacy' : 'Warehouse'}: ${activeEntity.name}`
                        : "Employees"
                }
                subtitle={
                    activeEntity 
                        ? `Manage ${activeEntity.type === 'shop' ? 'pharmacy' : 'warehouse'} staff, payroll, and assignments`
                        : "Manage staff, payroll, and assignments"
                }
                actions={
