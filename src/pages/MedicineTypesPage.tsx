...
export default function MedicineTypesPage() {
    ...
    const handleToggleStatus = async (item: MedicineType) => {
        try {
            await mastersApi.updateMedicineType(item.id, { is_active: !item.is_active });
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (editing) {
                ...
                handleSuccess('Medicine type updated successfully');
            } else {
                await mastersApi.createMedicineType(formData);
                handleSuccess('Medicine type created successfully');
            }
            setShowModal(false);
            loadData();
        } catch (err) {
    ...
    const confirmDelete = async () => {
        ...
        try {
            ...
            handleSuccess('Medicine type deleted successfully');
            loadData();
        } catch (err) {
            handleError(err, 'Failed to delete medicine type');
        } finally {
    ...
    return (
        <UniversalListPage>
            ...
            <Drawer
                ...
                title={editing ? 'Edit Medicine Type' : 'Add Medicine Type'}
                subtitle={editing ? 'Update medicine type information' : 'Create a new medicine type master'}
                width="md"
                footer={
                    <div className="flex justify-end gap-3">
                        ...
                        <Button variant="primary" type="submit" form="medicine-type-form" loading={saving}>
                            {saving ? 'Saving...' : editing ? 'Update' : 'Create'}
                        </Button>
                    </div>
                }
            >
                <form id="medicine-type-form" onSubmit={handleSubmit} className="space-y-5">
                    {error && (
